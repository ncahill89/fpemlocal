...
for(div in divs){
  ...
  if (div %in% unique(global$division_numeric_code)) {
...
pdf("married_comparison.pdf", 10,10)
i <- 0
...
# Plot unmarried
```{r}
global <- fpemdata::global_estimates_unmarried %>%
  dplyr::rename(division_numeric_code = Iso)
pl <- list()
i <- 0
for(div in divs){
...
pdf("unmarried_comparison.pdf", 10,10)
i <- 0
for(divname in divnames) {
  i <- i + 1
