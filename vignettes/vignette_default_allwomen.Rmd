...
1. Run the model `fpem_one_country`
2. Calculate point estimates `fpem_results`
...
## <a name="run"></a>
## Run models
`fpem_one_country` is a wrapper function to run the family planning estimation model. The argument `is_in_union` specifies which model we wish to run. There are two models, one for in-union and another for not-in-union women. These are indicated with `"Y"` and `"N"` respectively. We can also supply `"ALL" `to this argument to obtain results for all women. In this case both models are run and the posterior samples are combined to obtain posterior samples for all women.
```{r}
div <- 4
...
`fpem_one_country` returns a list runs. In this case where `"ALL"` women were requested, three lists are returned. Each with identical structure. These lists contain posterior samples and another list called `core_data`. Core data contains processed survey data and run specific data such as the time frame, union, etc.
```{r}
runlist$runy$core_data %>% names
```
