#' Fit one country model autosave
#'
#' A wrapper for \code{\link{fit_fp_c}} augmented with automatic saving.
#'
#' @param runname \emph{\sQuote{Character}} The name given to the file generated by this wrapper.
#' @param ... Please see \code{\link{fit_fp_c}} for additional arguments.
#'
#' @export
...
#'
#' @param survey_dataset \emph{\sQuote{Data frame}} If this argument is not specified the imputed version of the UNPD dataset \code{\link{contraceptive_use}} is used.
...
#' @param service_stats_dataset \emph{\sQuote{Data frame}}
...
#' @export
fit_fp_c <-
  function(survey_dataset = imputed_data$contraceptive_use_imputed,
           division_numeric_code = NULL,
           is_in_union = "Y",
           first_year = NULL,
           last_year = NULL,
           subnational = FALSE,
  ...
  if (is_in_union == "ALL") {
    ...
    fitr <- list(fit_fp_csub(survey_dataset,
                             division_numeric_code,
                             is_in_union,
                             first_year,
                             last_year,
...
#' Fit one country on union model
#'
#' The central subroutine for model fitting. Fits a single country model for in-union or not-in-union women.
#'
...
fit_fp_csub <- function(
  ...
  # indicate if use supplied custom data
  custom_data_indicator <- !identical(survey_dataset,imputed_data$contraceptive_use_imputed)
  # check inputs to this wrapper
  check_inputs(
    custom_data_indicator = custom_data_indicator,
  ...
  list_service_stats <-
    list_service_stats(
      service_stats_dataset = service_stats_dataset,
