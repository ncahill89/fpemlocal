...
list_auxiliary_data <- function(core_data) {
  if(nrow(core_data$observations) == 0) {
    ...
    getj.training.modonly.k <- seq(1,nrow(core_data$observations))[!is.na(core_data$observations$contraceptive_use_modern) & 
    ...
    data = list(
      ...
      modern = modern,
      unmet = unmet,
      logit.ymodonly.j = logit(modern),
      getj.training.modonly.k = getj.training.modonly.k,
      ...
      ratios.trad.modern.jn = cbind(log(trad/(1-modern-trad)), log(modern/(1-modern-trad))), # old order, trad goes first 
      logitratio.yunmet.j = log(unmet/(1-modern-trad - unmet))
